...
- **不做控制面 GUI**。~~第一版是库 + MCP server,控制面走各宿主现有会话~~
  —— 2026-09-24 修订:原条款是"不做 GUI",用户要一个给人看进度/分发/用量的人类视图,
  所以开了一个**只读**口子并**保留原决策的控制面部分**。边界写死三条(`src/ui.ts`):
  只听 127.0.0.1、端口交给系统分配(登记在 `~/.llms-bridge/ui.json`)、**没有任何派发/取消/合并接口**。
  理由:面板一旦能改状态,它就成了第二个主脑 —— 而它比主脑更没有上下文。
  派发仍然只走 MCP 六原语。
...
4. ~~三家投票评审~~ 与 **端到端闭环**:
   ...
   - **模型级分歧的理由已读到(2026-09-21)**。跑法改了:deepseek 的 Claude Code 额度断了,
     于是 **codex 生产 → codebuddy 一家、两个模型并发评审**(`hy4-preview` vs `deepseek-v4.1-flash`,
     后者经腾讯登录态拿到,**零额度**)。三家投票那条用例仍待 deepseek 充值后重跑。
     两家**结论一致(都不 LGTM)、深度差很多**:

     | | hy4-preview | deepseek-v4.1-flash |
     |---|---|---|
     | 抓到 `.pyc` 混入 diff | ✅ blocker | ✅ 并指出"随 Python 版本/机器漂移" |
     | 抓到失效注释 | ✅ | ✅ |
     | 去读真实文件核对 | ❌ 只读 diff | ✅ 核出"源仓库未改,改动只在 worktree" |
     | 声明自己未验证的部分 | — | ✅ "Read 拒绝二进制,这一点我未验证,不假装知道" |
     | 额外发现 | — | ✅ "这行改动没有任何证据支撑,只有'看起来对'" |

     最后一条是本次最硬的收获:**生产者自称验证过 `add(2,3)==5`,但 diff 里没有任何测试痕迹**
     (唯一痕迹就是那个 `.pyc`),评审方从"无证据"反推出其自述不可信。
     **由此两条**:① 桥自身缺陷 —— `captureDiff` 的 `git add -A` 把 worker 的构建产物当交付物回收,
     两个独立评审方都判它 blocker(见任务 #16);② 措辞纪律 —— 这次是**同一家 CLI 的两个模型**,
     它证明的是"换模型能产生分歧",**不是**"跨厂商";跨厂商仍由 codex→codebuddy 那条用例守着。
     也别把"两家一致"当结论:谁对要读原文,不是数票。
